...
class BDA(nn.Module):
    def __init__(self, config, att, init_w2v_att, seenclass, unseenclass,
        ...
        self.transformer = Transformer(
            ...
            dropout1=config.tf_dropout1,
            alpha = config.alpha,
            beta = config.beta,
            fsqrt = config.fsqrt,
            sqrt=config.sqrt,
            weight=config.weight,
            weight_self=config.weight_self,
            tgt_weight= config.tgt_weight,
            SAtt=config.tf_SAtt,
    ...
    def compute_loss(self, in_package):
        ...
        loss =   loss_CE + self.config.lambda_ * \
            loss_cal + self.config.lambda_reg * loss_reg + self.config.lambda_CEVi * loss_CE_Ve
...
class Transformer(nn.Module):
    def __init__(self, ec_layer=1, dc_layer=1, dim_com=300,
                 dim_feedforward=2048, dropout=0.1,dropout1=0.1,alpha=0,beta=1,fsqrt=150,
                 sqrt=150,weight=0.001,weight_self=0.001,
                 tgt_weight=1.5,  heads=1,
...
class EncoderLayer(nn.Module):
    def __init__(self, d_model=512, d_k=64, d_v=64, h=8, d_ff=2048,
                 dropout=.1,dropout1=.1,alpha=0,beta=1,fsqrt=150,sqrt=150,weight=0.001,
                  weight_self=0.001, identity_map_reordering=False,
                 attention_module=None, attention_module_kwargs=None):
        super(EncoderLayer, self).__init__()
        self.identity_map_reordering = identity_map_reordering
        self.mhatt = MultiHeadGeometryAttention(d_model, d_k, d_v, h, dropout,dropout1,fsqrt,weight,
                                                identity_map_reordering=identity_map_reordering,
                                                attention_module=attention_module,
                                                attention_module_kwargs=attention_module_kwargs)
        self.mhatt2 = MultiHeadGeometryAttention_self(d_model, d_k, d_v, h, dropout,sqrt,weight_self,
                                                identity_map_reordering=identity_map_reordering,
                                                attention_module=attention_module,
                                                attention_module_kwargs=attention_module_kwargs)
        self.alpha = alpha
    ...
    def forward(self, queries, keys, values,l2, relative_geometry_weights,
        ...
        att1 = self.mhatt2(att, l2, l2,
                         attention_mask, attention_weights)
        att2 = self.lnorm(att + self.dropout(att1))
        # att = att1+ att2            
...
class MultiLevelEncoder_woPad(nn.Module):
    def __init__(self, N, d_model=512, d_k=64, d_v=64, h=8, d_ff=2048,
        ...
        self.layers = nn.ModuleList([EncoderLayer(d_model, d_k, d_v, h, d_ff, dropout,dropout1,alpha,beta,fsqrt,
                                                  sqrt,weight,weight_self,
                                                  identity_map_reordering=identity_map_reordering,
                                                  attention_module=attention_module,
                                                  attention_module_kwargs=attention_module_kwargs)
                                     for _ in range(N)])
    ...
    def forward(self, input, l2, attention_mask=None, attention_weights=None, pos=None):
        ...
        # relative_geometry_embeddings size : torch.Size([50, 196, 196, 64])
        flatten_relative_geometry_embeddings = relative_geometry_embeddings.view(
            -1, 64)
        # flatten_relative_geometry_embeddings size :  torch.Size([1920800, 64])
        box_size_per_head = list(relative_geometry_embeddings.shape[:3])
#       box_size_per_head [list]: [50, 196, 196]
#       box_size_per_head(insert(1,1) after:  [50, 1, 196, 196]
        ...
        return out
# relative_geometry_weights_per_head [0]size():  torch.Size([50, 1, 196, 196])
# relative_geometry_weights_per_head len:  1

class TransformerDecoderLayer(nn.TransformerDecoderLayer):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, tgt_weight=1.5,
...
class ScaledDotProductGeometryAttention(nn.Module):
    def __init__(self, d_model, d_k, d_v, h, dropout=.1,dropout1=.1,fsqrt=150,weight=1, comment=None):
    ...
    def forward(self, queries, keys, values, box_relation_embed_matrix,
        ...
        v = v * self.weight
        v = self.dropout1(v)                                 
        att = torch.matmul(q, k) / np.sqrt(self.fsqrt)  # self.d_k
...
class ScaledDotProductGeometryAttention_self(nn.Module):
    def __init__(self, d_model, d_k, d_v, h, dropout=.1,sqrt=150,weight_self=1, comment=None):
    ...
    def forward(self, queries, keys, values, 
        ...
        k = self.fc_k(keys).view(b_s, nk, self.h, self.d_k).permute(0, 2, 3, 1)
        k = k * self.weight_self
        v = self.fc_v(values).view(b_s, nk, self.h,
                                   self.d_v).permute(0, 2, 1, 3)
        v = v * self.weight_self
        att = torch.matmul(q, k) / np.sqrt(self.sqrt)
        if attention_weights is not None:
            att = att * attention_weights
        if attention_mask is not None:
            att = att.masked_fill(attention_mask, -np.inf)
...
class MultiHeadGeometryAttention_self(nn.Module):
    def __init__(self, d_model, d_k, d_v, h, dropout=.1,sqrt=150,weight_self=0.001,
        ...
        self.attention = ScaledDotProductGeometryAttention_self(
            d_model=d_model, d_k=d_k, d_v=d_v, h=h,sqrt=sqrt,weight_self=weight_self, comment=comment)
...
class MultiHeadGeometryAttention(nn.Module):
    def __init__(self, d_model, d_k, d_v, h, dropout=.1,dropout1=.1,fsqrt=150,weight=0.001, identity_map_reordering=False,
                 can_be_stateful=False, attention_module=None,
                 attention_module_kwargs=None, comment=None):
        super(MultiHeadGeometryAttention, self).__init__()
        self.identity_map_reordering = identity_map_reordering
        self.attention = ScaledDotProductGeometryAttention(
            d_model=d_model, d_k=d_k, d_v=d_v, h=h,dropout=dropout,dropout1=dropout1,fsqrt=fsqrt,weight=weight, comment=comment)
        self.dropout = nn.Dropout(p=dropout)
        self.layer_norm = nn.LayerNorm(d_model)
        self.can_be_stateful = can_be_stateful
        if self.can_be_stateful:
